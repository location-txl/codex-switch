...
第三方 provider 使用各自保存的 API key，通过 `codex-switch token <name>` 提供给 Codex 运行时；切换第三方 provider 不需要 OpenAI 登录态。切回 OpenAI 官方 provider 时，CLI 会优先刷新 `~/.codex-switch/auth.json` 中保存的 OpenAI 登录态，并同步到 Codex 运行时 `auth.json`。
